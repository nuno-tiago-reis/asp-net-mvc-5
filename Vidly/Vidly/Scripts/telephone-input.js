...
(function(factory)
{
	...
	{
		window.define(["jquery"], function($) { factory($, window, document); });
	}
	else
	{
...
(
	function($, window, document, undefined)
	{
		...
		// checks a classes call as a function
		...
		// creates properties for a class
		function createProperties(target, properties)
		{
		...
		// creates a class using the given properties
		function createClass(Constructor, protoProperties, staticProperties)
		{
			if (protoProperties)
			{
				createProperties(Constructor.prototype, protoProperties);
			}
			if (staticProperties)
			{
				createProperties(Constructor, staticProperties);
			}
			return Constructor;
		}
		...
		{
			var countries = allCountries[i];

			allCountries[i] =
			{
				name: countries[0],
				iso2: countries[1],
				dialCode: countries[2],
				priority: countries[3] || 0,
				areaCodes: countries[4] || null
			};
		}
		...
		window.intlTelInputGlobals.loadUtils = function(path)
		{
			...
			if (!window.intlTelInputUtils && !window.intlTelInputGlobals.startedLoadingUtilsScript)
			{
				...
				{
					return new Promise(function(resolve, reject) { return injectScript(path, resolve, reject); });
				}
				injectScript(path);
			}
			return null;
		...
		// run a method on each property of the object
		var forEachProperty = function forEachProperty(obj, callback)
		{
		...
		{
			forEachProperty(window.intlTelInputGlobals.instances, function(key)
			{
		...
		var TelephoneInput = /*#__PURE__*/ function()
		{
			function TelephoneInput(input, options)
			{
				var _this = this;
				...
				this.options = {};
				forEachProperty(defaults, function(key, value) { _this.options[key] = customOptions.hasOwnProperty(key) ? customOptions[key] : value; });
				...
				[
					...
					},
					{
						...
						value: function _processAllCountries()
						{
							...
							{
								var lowerCaseOnlyCountries = this.options.onlyCountries.map(function(country) { return country.toLowerCase(); });
								this.countries = allCountries.filter(function(country) { return lowerCaseOnlyCountries.indexOf(country.iso2) > -1; });
							}
							else if (this.options.excludeCountries.length)
							{
								var lowerCaseExcludeCountries = this.options.excludeCountries.map(function(country) { return country.toLowerCase(); });
								this.countries = allCountries.filter(function(country) { return lowerCaseExcludeCountries.indexOf(country.iso2) === -1; });
							}
							else
							{
					...
					},
					{
						...
						value: function _countryNameSort(a, b) { return a.name.localeCompare(b.name); }
					},
					{
					...
					},
					{
						...
						value: function _createEl(name, attrs, container)
						{
							...
							if (attrs)
							{
								forEachProperty(attrs, function(key, value) { return el.setAttribute(key, value); });
							}
							if (container)
							{
					...
					},
					{
						...
						{
							var _this3 = this;
							this._handleHiddenInputSubmit = function() { _this3.hiddenInput.value = _this3.getNumber(); };
						...
						value: function _initRequests()
						{
							...
							if (this.options.utilsScript && !window.intlTelInputUtils)
							{
								...
								else
								{
									...
									window.addEventListener("load", function() { window.intlTelInputGlobals.loadUtils(_this5.options.utilsScript); });
								}
							}
							else
							{
								...
								if (typeof this.options.geoIpLookup === "function")
								{
									...
									(
										function(countryCode)
										{
											...
											setTimeout(function() { return forEachInstance("handleAutoCountry"); });
										},
										function() { return forEachInstance("rejectAutoCountryPromise"); });
								}
							}
						}
					},
					...
					},
					{
						...
						value: function _initFocusListeners()
						{
							...
							this._handleSubmitOrBlurEvent = function() { _this7._removeEmptyDialCode(); };
							if (this.telInput.form)
							{
					...
					{
						key: "_getNumeric",
						value: function _getNumeric(s) { return s.replace(/\D/g, ""); }
					},
					{
						key: "_trigger",
						...
						value: function _setDropdownPosition()
						{
							...
							if (!this.isMobile)
							{
								...
								if (this.options.dropdownContainer)
								{
									...
									this._handleWindowScroll = function() { return _this8._closeDropdown(); };
					...
					},
					{
						...
						value: function _bindDropdownListeners()
						{
							...
							this._handleMouseoverCountryList = function(e)
							{
								...
								if (listItem)
								{
									_this9._highlightListItem(listItem);
								}
							};
					...
					},
					{
						...
						value: function _updateValFromNumber(originalNumber)
						{
							...
							if (this.options.formatOnDisplay && window.intlTelInputUtils && this.selectedCountryData)
							{
								...
								var _intlTelInputUtils$nu = window.intlTelInputUtils.numberFormat, NATIONAL = _intlTelInputUtils$nu.NATIONAL, INTERNATIONAL = _intlTelInputUtils$nu.INTERNATIONAL;
								var format = useNational ? NATIONAL : INTERNATIONAL;
								number = window.intlTelInputUtils.formatNumber(number, this.selectedCountryData.iso2, format);
							}
					...
					},
					{
						...
						value: function _updatePlaceholder()
						{
							...
							{
								var numberType = window.intlTelInputUtils.numberType[this.options.placeholderNumberType];
								var placeholder = this.selectedCountryData.iso2 ? window.intlTelInputUtils.getExampleNumber(this.selectedCountryData.iso2, this.options.nationalMode, numberType) : "";
					...
					},
					{
						...
						value: function getExtension()
						{
							if (window.intlTelInputUtils)
							{
								return window.intlTelInputUtils.getExtension(this._getFullNumber(), this.selectedCountryData.iso2);
							}
							return "";
						}
					},
					{
						...
						value: function getNumber(format)
						{
							if (window.intlTelInputUtils)
							{
								...
								return window.intlTelInputUtils.formatNumber(this._getFullNumber(), iso2, format);
							}
							return "";
						}
					},
					{
						...
						value: function getNumberType()
						{
							if (window.intlTelInputUtils)
							{
								return window.intlTelInputUtils.getNumberType(this._getFullNumber(), this.selectedCountryData.iso2);
							}
							return -99;
						}
					},
					...
					},
					{
						...
						value: function isValidNumber()
						{
							...
							return window.intlTelInputUtils ? window.intlTelInputUtils.isValidNumber(val, countryCode) : null;
						}
					},
					{
						key: "setCountry",
			...
			if (options === undefined || typeof options === "object")
			{
				...
				(
					function()
					{
						...
						{
							var telephoneInput = new TelephoneInput(this, options);
							telephoneInput._init();
							window.intlTelInputGlobals.instances[telephoneInput.id] = telephoneInput;
							$.data(this, "plugin_" + pluginName, telephoneInput);
						}
					}
				);
			}
			else if (typeof options === "string" && options[0] !== "_")
			{
				...
				(
					function()
					{
						...
						if (instance instanceof TelephoneInput && typeof instance[options] === "function")
						{
